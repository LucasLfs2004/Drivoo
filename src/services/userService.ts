...
const createBaseProfile = (papel: UserRole, apiUser: ApiCurrentUser): Usuario['perfil'] => {
  if (papel === 'instrutor') {
    return {
...
export const mapApiUserToUsuario = (
  ...
  return {
    ...
    papel,
    perfil: previousUser?.papel === papel
      ? {
          ...previousUser.perfil,
          primeiroNome: apiUser.nome,
          ultimoNome: apiUser.sobrenome,
...
export const userService = {
  ...
  async updateCurrentUser(userId: string, data: UpdateCurrentUserPayload): Promise<Usuario> {
